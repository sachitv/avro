...
/**
 * Compute MD5 digest for the provided UTF-8 string.
 * Only `md5` is currently supported in the browser bundle.
 *
 * @param {string} str
...
function md5(buffer) {
  ...
  words[(paddedLength >>> 2) - 2] = originalBitLength & 0xffffffff;
  words[(paddedLength >>> 2) - 1] = (originalBitLength / 0x100000000) >>> 0;
  ...
  writeUInt32LE(out, a, 0);
  writeUInt32LE(out, b, 4);
  writeUInt32LE(out, c, 8);
  writeUInt32LE(out, d, 12);
  return out;
}

function writeUInt32LE(dst, value, offset) {
