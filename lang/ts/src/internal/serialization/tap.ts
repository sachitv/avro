...
export class Tap {
  ...
  async _testOnlyBuf(): Promise<Uint8Array> {
    const bytes = await this.#buffer.read(0, this.#buffer.length);
  ...
  async matchString(tap: Tap): Promise<number> {
    ...
    const p2 = tap.#pos;
    tap.#pos += l2;
    const len1 = getClampedLength(this.#buffer.length, p1, l1);
    const len2 = getClampedLength(tap.#buffer.length, p2, l2);
