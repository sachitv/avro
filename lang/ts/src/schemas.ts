...
import { format, inherits } from './util.ts';
import {
  alloc,
  allocUnsafeSlow,
...
import type { BinaryBuffer } from './binary.ts';
...
var TAP = new Tap(allocUnsafeSlow(1024));
...
Type.__reset = function (size) { TAP.buf = allocUnsafeSlow(size); };
...
Type.prototype.toBuffer = function (val) {
  ...
  var buf = alloc(TAP.pos);
  copyBinary(TAP.buf, buf, 0, 0, TAP.pos);
  return buf;
};
...
BytesType.prototype._copy = function (obj, opts) {
  ...
  switch ((opts && opts.coerce) | 0) {
    case 3: // Coerce buffers to strings.
      ...
      return toBinaryString(obj as BinaryBuffer);
    case 2: // Coerce strings to buffers.
      if (typeof obj != 'string') {
        throw new Error(f('cannot coerce to buffer: %j', obj));
      }
      buf = binaryFrom(obj, 'binary');
    ...
    case 1: // Coerce buffer JSON representation to buffers.
      ...
      buf = binaryFrom(obj.data);
      this._check(buf, throwInvalidError);
      return buf;
    default: // Copy buffer.
      this._check(obj, throwInvalidError);
      return binaryFrom(obj as BinaryBuffer);
  }
};
BytesType.prototype.compare = compareBinary;
...
FixedType.prototype._check = function (val, cb) {
  var b = isBinary(val) && (val as BinaryBuffer).length === this._size;
  if (!b && cb) {
    cb(PATH.slice(), val, this);
  }
  return b;
...
FixedType.prototype._write = function (tap, val) {
  if (!isBinary(val) || (val as BinaryBuffer).length !== this._size) {
    throwInvalidError(null, val, this);
  }
  tap.writeFixed(val as BinaryBuffer, this._size);
};
...
FixedType.prototype.compare = compareBinary;
...
RecordType.prototype._createWriter = function () {
  ...
  for (i = 0, l = this._fields.length; i < l; i++) {
    ...
    if (field.getDefault() === undefined) {
      ...
      var defaultBuffer = field._type.toBuffer(field.getDefault()) as BinaryBuffer;
      value = toBinaryString(defaultBuffer);
