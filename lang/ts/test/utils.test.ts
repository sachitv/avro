...
function fromHexStrings(values: string[]): Uint8Array {
  return makeBuffer(values.map((value) => {
...
function toSignedBytes(values: number[]): Uint8Array {
  return makeBuffer(values.map((value) => (value + 256) % 256));
}

function binaryEqualArrays(actual: Uint8Array, expected: number[] | Uint8Array): void {
  const target = expected instanceof Uint8Array ? toArray(expected) : expected;
  assert.deepEqual(toArray(actual), target);
}
  ...
  describe('Tap', function () {
    ...
    describe('int & long', function () {
      ...
      it('write', function () {
        ...
        var buf = fromHexStrings(['0xd8', '0xce', '0x80', '0xbc', '0xee', '0x53']);
        assert(tap.isValid());
        assert.deepEqual(toArray(tap.buf), toArray(buf));
    ...
    describe('bytes', function () {
      ...
      testWriterReader({
        elems: [makeBuffer('abc', 'binary'), allocBuffer(0), makeBuffer([1, 5, 255])],
    ...
    describe('binary', function () {
      ...
      it('write valid', function () {
        ...
        var s = '\x01\x02';
        tap.writeBinary(s, 2);
        binaryEqualArrays(tap.buf, [1, 2, 0]);
      });
    ...
    describe('pack & unpack longs', function () {
      ...
      it('unpack single byte', function () {
        ...
        binaryEqualArrays(
          t.unpackLongBytes(),
          toArray(toSignedBytes([-5, -1, -1, -1, -1, -1, -1, -1]))
        );
        t.pos = 0;
      });
      ...
      it('unpack multiple bytes', function () {
        ...
        t.writeLong(l);
        t.pos = 0;
        assert.equal(readInt32LE(t.unpackLongBytes()), l);
        t.pos = 0;
        l = -3210984;
        t.writeLong(l);
        t.pos = 0;
      ...
      it('pack single byte', function () {
        ...
        writeInt32LE(b, -1, 4);
        t.packLongBytes(b);
        binaryEqualArrays(t.buf.slice(0, t.pos), [1]);
        t.pos = 0;
      ...
      it('roundtrip', function () {
        ...
        function roundtrip(n) {
          ...
          assert.deepEqual(toArray(t2.buf), toArray(t1.buf));
        }
      });
